<template>
  <div>
    <v-toolbar flat color="white">
      ...
      <v-dialog v-model="deleting" persistent width="500">
        <v-card>
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="primary" flat @click="eliminarEmpleado(false)">
              No
            </v-btn>
            <v-btn color="primary" flat @click="eliminarEmpleado(true)">
              Si
            </v-btn>
          </v-card-actions>
        </v-card>
      ...
      <v-dialog v-model="dialog" persistent max-width="500px">
        ...
        <v-card>
          ...
          <v-card-text>
            <v-form :key="form" v-model="valid">
              <v-container grid-list-md>
                <v-layout wrap>
                  <v-flex xs12 md6>
                    <v-text-field v-model="empleado.nombres" :rules="validaciones.nombres" label="Nombres"></v-text-field>
                  </v-flex>

                  <v-flex xs12 md6>
                    <v-text-field v-model="empleado.apellidos" :rules="validaciones.apellidos" label="Apellidos"></v-text-field>
                  </v-flex>
                  ...
                  <v-flex xs12 md6>
                    <v-text-field v-model="empleado.telefono" :rules="validaciones.telefonos" label="Telefono"></v-text-field>
                  </v-flex>

                  <v-flex xs12 md6>
                    <v-text-field v-model="empleado.celular" :rules="validaciones.telefonos" label="Celular"></v-text-field>
                  </v-flex>

                  <v-flex xs12 md6>
                    <v-select v-model="empleado.genero" :items="genero" :rules="validaciones.requerido" item-text="genero" item-value="codigo_genero" label="Genero"></v-select>
                  </v-flex>
          ...
          <v-card-actions>
            ...
            <v-btn v-if="empleadoActivo == -1" color="blue darken-1" flat @click="save" :disabled="!valid">Guardar</v-btn>
    ...
    <v-snackbar v-model="deleted" :color="'error'" :timeout="3000">El empleado se ha eliminado correctamente <v-btn dark flat @click="deleted = false"> Cerrar </v-btn> </v-snackbar>
    <v-snackbar v-model="updated" :color="'success'" :timeout="3000">El empleado se ha actualizado correctamente <v-btn dark flat @click="updated = false"> Cerrar </v-btn> </v-snackbar>
    <v-snackbar v-model="exist" :color="'error'" :timeout="3000">El empleado con dpi {{ empleado.dpi }} ya existe <v-btn dark flat @click="exist = false"> Cerrar </v-btn> </v-snackbar>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      eliminarEmpleado(evt) {
        this.deleting = false;
        if(evt){
          services.eliminarEmpleado({codigo_empleado: this.empleadoActivo}).then(response =>{
            this.eliminado = true;
            this.reload();
          }, errorResponse =>{
              console.error(errorResponse);
